classdef arduino < handle
    ...
    % Modified by R White many times ... most recently 6/3/2020
    % Added Grove HX711 loadcell support
    ...
    methods
        ...
        function a=arduino(comPort)
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                if isempty(chk)
                    delete(a);
                    error('Connection unsuccessful, please make sure that the Arduino is powered on, running rdwsrv.ino, and that the board is connected to the indicated serial port. You might also try to unplug and re-plug the USB cable before attempting a reconnection.');
                end
            ...
            a.pins=-1*ones(1,19);
            %ROB Modification 1/4/2012 if using Ardumoto with output pins
            %3,11,12,13
            %and quad encoder A and B on 2 and 4
            a.pins([3 11 12 13])=1;
            a.pins([2 4])=0;
            
            %ROB Modification 6/3/2020 if using Grove sensor base with
            %MKR WiFi1010 and HX711 loadcell
            a.pins([5 6])=1;
        ...
        %%%Rob White added these additional methods for use with the 
        %%%ARUMOTO board and quadrature encoder reader:
                
        % Quadrature Encoder Read
        function val=encoderRead(a,valType)
            
            % val=a.encoderRead(valType); performs quad encoder reading
            % The first argument before the function name, a, is the arduino object.
            % The argument valType, is the type of quadrature reading
            % desired.  It can be 'position' in which case position in
            % counts (1 count = 1 degree for NXT motor), 'filtvel' in which
            % case velocity is returned with a 10 Hz low pass filter
            % applied on the Arduino board, 'rawvel' in which case
            % unfiltered velocity is returned.  Both velocities are in
            % counts per second. 'zero' in which case board zeros out
            % encoder and velocity state at current location
            %
            % Example:
            % val=a.encoderRead('position'); % reads angular position
            % val=a.encoderRead('filtvel'); %reads filtered angular
            % velocity (10 Hz low pass filter implemented on Arduino)
            % val=a.encoderRead('rawvel'); %raw instantaneous velocity with
            % no filter (this will be noisy!)
            % a.encoderRead('zero'); %board sets current position to zero
            % and velocity state to zero (for velocity filter)
            % 
            ...
            if a.chkp,
                ...
                % check nargin
                if nargin~=2,
                    error('Function must have the "valType" argument');
                end
                
                % check options for valType
                if ~strcmp(valType,'position') && ~strcmp(valType,'filtvel') && ~strcmp(valType,'rawvel') && ~strcmp(valType,'zero'),
                    error('valType must be "position" "filtvel" "rawvel" or "zero"');
                end
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % send mode and valtype
                if strcmp(valType,'position'),
                    fwrite(a.aser,[53 97],'uchar');
                elseif strcmp(valType,'filtvel'),
                    fwrite(a.aser,[53 98],'uchar');
                elseif strcmp(valType,'rawvel'),
                    fwrite(a.aser,[53 99],'uchar');
                elseif strcmp(valType,'zero'),
                    fwrite(a.aser,[53 100],'uchar');
                end
        ...
        % Motor Drive
        function motorDrive(a,motor,level)
            
            % val=a.motorDrive(motor,level); performs motor drive using Ardumoto.
            % The first argument before the function name, a, is the arduino object.
            % The second argument, motor, is either 1 (motor A) or 2 (motor B)
            % The third argument, level, is a level number which should lie
            % between -255 and +255 where 255 is full PWM on. 
            % The sign will be used to
            % determine how to drive the digital pin used to determine
            % direction
            
            % Example:
            % a.motorDrive(1,127); % drives motor 1 (motor A) forward at 50%
            % (127/255)
            % a.motorDrive(2,-255); % drives motor 2 (motor B) backward at
            % 100% (255/255)
            ...
            if a.chkp,
                ...
                % check nargin
                if nargin~=3,
                    error('Function must have the "motor" and "level" arguments');
                end
                
                % check options for motor
                if motor~=1 && motor~=2,
                    error('motor must be 1 (motor A) or 2 (motor B)');
                end
            ...
            if strcmpi(get(a.aser,'Port'),'DEMO'),
                ...
                % send mode, motor, and level
                if motor==1 && level>=0,
                    fwrite(a.aser,[54 97 level],'uchar');
                elseif motor==2 && level>=0,
                    fwrite(a.aser,[54 98 level],'uchar');
                elseif motor==1 && level<0,
                    fwrite(a.aser,[54 99 -level],'uchar');
                elseif motor==2 && level<0,
                    fwrite(a.aser,[54 100 -level],'uchar');
                end
        ...
        % load cell read
        function val=loadcellRead(a)
            
            % val=a.loadcellRead(); Performs loadcell read.
            % The first argument before the function name, a, is the arduino object.
            % Results read back will be a raw integer value.  A
            % calibration is required to change the integer to a load.
            % Example:
            % val=a.loadcellRead(); % reads the loadcell
            %
            ...
            if a.chkp,
                ...
                % check nargin
                if nargin~=1,
                    error('No additional arguments allowed to loadcellRead');
                end
